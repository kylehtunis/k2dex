...
7. **`notebooks/temporal_drift.ipynb`** — standalone (terminal, not a pipeline dependency). Temporal drift analysis: how quickly does model accuracy degrade as the meta evolves? Chronological 25/75 split (low train fraction buys wide temporal window); 8 test windows. Uses `k2dex.models.fit_pl_ising` and `k2dex.limitless_ingest.chronological_split`.
8. **`notebooks/energy_discrimination.ipynb`** — standalone validation (terminal, not a pipeline dependency). Scores held-out real teams vs. a marginal-usage null under each fit, decomposes Score into popularity (`h·s`) and coherence (`ΣJ`), and sweeps `field_weight` for the AUC-optimal Bias Adjustment (peaks ~0.25–0.30 — real-vs-null discrimination lives in the pairwise term; the popularity prior is complementary but dilutes at full weight). Chronological 90/10 split; uses `k2dex.{models.fit_pl_ising, sampling.team_energy, rendering.intra_team_sum_j}`.
9. **`notebooks/outcome_validation.ipynb`** — standalone validation (terminal, not a pipeline dependency). Tests whether the model's Score/coherence predicts *tournament placement* (not just team membership). Requires cache schema ≥ v2 (placing + record); uses `limitless_ingest.all_teams_with_outcomes`. Fits both models on the **full corpus** (placement is never an input to `(J,h)`, so in-sample scoring isn't leakage — argued in the notebook), then relates per-team coherence/popularity/Score to within-tournament normalized placement via pooled + within-event Spearman and coherence-quintile conditional-quantile plots (the hypothesis is heteroscedastic: high coherence widens the placement spread rather than just shifting the mean). Ends with an out-of-time refit as a robustness check. Uses `k2dex.models.fit_pl_ising`.
...
- **`rendering_html.py`** — HTML-string builders for the lab-notebook visual language. Atoms: `section_label`, `stat`, `stat_strip`, `score_chip`, `signed_bar`, `mini_bar`, `corpus_cell` (merged obs / Δswaps badge). Sprite primitives: `sprite_img` / `_sprite_box` (return `<object>` elements with native HTML5 fallback — Streamlit strips inline event handlers so `<object>` is the only viable path; see "Webapp visual language" gotchas). Vocab parsing: `extract_species` / `extract_item` to split `"Species @ Item"` strings. Composed cells: `slot_card` / `slot_card_empty` / `slot_strip`, `comp_mon_cell`, `pair_cell`, `swap_cell`, `inline_mon`, `team_mini_strip`. Class names are paired with `styles.py` — renaming one means renaming both.
...
- **`limitless_ingest.py`** — fetches recent VGC Reg M-A tournaments from `play.limitlesstcg.com/api` (paginated, with name + Protect-ratio singles filters), extracts parsed teams and caches one small JSON per tournament under `tournaments_cache/`. A team is a `Team` record: the `(species, item)` roster frozenset (`members`, the model feature) plus its tournament outcome — `placing` (1 = winner; None when the API omits it) and the win/loss/tie `record`. `all_teams(tournaments)` projects to roster frozensets (the fitting-path view); `all_teams_with_outcomes(tournaments)` keeps the full `Team` records for outcome validation (coherence/Score vs. placement). Also exports `chronological_split(tournaments, train_frac)` which sorts tournaments by date and splits at the tournament boundary nearest to `train_frac` of total teams — used by all three validation notebooks. Walks newest-first until `>= min_teams` (default `PHASE2_MIN_TEAMS = 25000`, aspirational — currently exhausts available tournaments before reaching it) teams accumulate. Cache schema is versioned (`CACHE_VERSION`, now 2 — placing/record added); bump when parsing logic changes. `normalize_name` collapses case/whitespace variants in the raw API; `strip_mega_prefix` collapses `Mega <X>` species names (with optional X/Y/Z forme suffix) down to the base species — the held Mega Stone is the source of truth for which mega forme, and players were inconsistent about whether they prefixed the species name.
...
Post-v0.4 the app uses a "lab notebook" palette + type system delivered via CSS injection. **Brand accent color is forest green: `--lab-accent: oklch(0.42 0.10 145)` in `tokens.css` / `LAB_ACCENT` in `styles.py`.** It must appear on every page: active tab underline, active model card border, focused select outline, primary button fill, minibar fills, and `/science` act-heading dividers. Use `var(--lab-accent)` in new CSS rather than hardcoding a hex. On the `/science` page, interactive simulation controls (Play/Pause/Step/Reset/New-graph) use `.lab-science-btn` from `widgets.css` — an outlined button that transitions to the accent on hover. Do NOT write bare `<button>` elements in the science sections. Five non-obvious things to know before touching it:

- **Streamlit strips inline event handlers and won't execute `<script>` tags injected via `st.markdown`.** This is why `sprite_img` (Streamlit/`rendering_html.py`) returns an HTML5 `<object>` with an inner `<span>` fallback — when the `data=` URL fails, the browser natively renders the inner content. We tried `onerror`-swap (handler stripped), layered `background-image` (transparent sprites let the missingno bleed through), and inline `<script>` rebinding (also blocked). `<object>` was the only path that worked on the Streamlit side. The React `Sprite.tsx` uses `<img onError>` instead — React's synthetic event system handles it fine — and additionally renders a bottom-right item-icon overlay for Species @ Item features.
...
- **Regularization differs by model.** The species model fit uses `SPECIES_LR_C = 0.1`; the species+item model fit uses `SPECIES_ITEM_LR_C = 1.0` — the wider, sparser pair feature matrix wants weaker regularization (higher `C`). Each `loaders` builder passes its own constant; `fit_pl_ising`'s `C` default is a neutral literal (`0.1`), not a model constant, since the function is generic over a single `C`. `precompute.py` records the per-model `C` in `meta.json` via `MODEL_LR_C`. `notebooks/regularization_sweep.ipynb` is the sweep harness behind these values.
...
- **`PHASE2_MIN_TEAMS = 25000`** is the target corpus size for ingest; the ingestor walks newest-first accumulating teams until it reaches this many. This number is **aspirational** — fewer than 25000 teams currently meet the ingest filters, so the walk exhausts the available tournaments rather than hitting the cap. Raising it past the available supply is a no-op until more qualifying tournaments exist.
...
- **Completer mode + Bias Adjustment defaults.** The webapp `/completer` defaults to the full PT sampler (`usePT=true`) at Bias Adjustment `0.3`; checking **Greedy sampling** flips to the fast MF-fill → greedy-descent path *and* bumps Bias Adjustment to `0.8`. Rationale: `energy_discrimination.ipynb` puts real-vs-null discrimination's optimum at `field_weight ≈ 0.25–0.30`, but greedy descent wanders into incoherent basins at low weight with few pins, so it needs the stronger popularity prior to stay sane. Toggling the checkbox sets the bias in *both* directions (greedy→0.8, full→0.3). Defaults live in `web/src/state/PageStateContext.tsx` (web) and the `value=` of the two `st.select_slider`s in `scripts/app.py` (Streamlit, both `0.3`). The Streamlit completer's 4-way technique radio (Parallel-tempered / Sample / Anneal / Greedy) is a different control and is left unchanged.
- **Clipboard import + shareable URLs (static webapp only).** Both `/completer` and `/analysis` have an "Import from clipboard" button that parses a pokepaste (`render/vocab-match.ts:parsePokepaste`/`matchPaste`) — only the first `Species @ Item` line of each block matters. Matching slugifies *both* the pasted name and each vocab entry's `speciesOf[i]` via the parity-gated `speciesToSlug`/`itemToSlug`, so Showdown forme naming (`Ninetales-Alola`) bridges to corpus naming (`Alolan Ninetales`). Before slugifying, the pasted name is mega-normalized (`render/vocab-match.ts:importSpeciesSlug`): a trailing `-Mega`/`-Mega-X`/`-Mega-Y`/`-Mega-Z` suffix is stripped (Showdown exports `Charizard-Mega-Y`, the corpus buckets it as base `Charizard` — the mega stone is the source of truth, mirroring `limitless_ingest.strip_mega_prefix` which collapses the API's *prefix* form), and a small `IMPORT_SPECIES_ALIASES` map handles formes that don't reduce by suffix-stripping alone (e.g. all three Showdown spellings of Mega Floette — `Floette`, `Floette-Mega`, `Floette-Eternal` — resolve to the corpus's `Eternal Flower Floette`, slug `floette`). Out-of-vocab species/items surface as gentle `.lab-form-note` warnings, not errors. Both pages also **live-sync their state into the URL** (`render/shareLink.ts`), debounced ~300ms so a Bias-slider drag settles into one write. State is encoded by slug (not raw index) so links survive `precompute.py` vocab reorders; the model id rides in the token (`s`/`si`) and a shared link auto-switches the model on open. The `/completer` token additionally carries excluded species, the greedy toggle, temperature, non-default Advanced-PT knobs, and the **RNG seed** of the displayed PT run — so a completer link reproduces the exact sampled teams when the recipient clicks Sample (greedy mode is already deterministic, so no seed). The legacy `/analysis?team=idx,…` and `/completer?pinned=species` params still decode for old links. No Python twin → no `tests/test_parity.py` rows.
...
- **Validation split is chronological 90/10** (`VALIDATION_TEAM_FRAC_TEST = 0.10`). Tournaments sorted oldest-first; the oldest ~90% of teams (by tournament boundary) form the train set via `limitless_ingest.chronological_split`. This is an out-of-time evaluation: the model is fit on past data and tested on future data. Vocab is built from train only — test teams with OOV members are dropped. `VALIDATION_SEED = 42` is still used for the within-test-set held-out position RNG (leave-k-out sampling), not for the split itself.
- **Temporal drift uses a separate 25/75 chronological split** (`DRIFT_TRAIN_FRAC = 0.25`). The low train fraction sacrifices absolute accuracy to give a wide temporal window (75% of the timeline) for observing drift, shocks, and relative model decay rates. Lives in its own notebook (`temporal_drift.ipynb`).
...
**Sprite rendering** (`web/src/render/Sprite.tsx`) uses `<img onError>` → missingno fallback. For Species @ Item features, a small item-icon badge overlays the bottom-right corner via `itemSpriteUrl()` in `sprite-url.ts` (PokeAPI sprites via jsDelivr CDN); the overlay hides itself gracefully if the item icon 404s. The Showdown CDN URL builder + slug rules live in `sprite-url.ts` and mirror `rendering_html.species_to_slug`.
...
- **`GraphView` is sprite-capable.** Two node render modes beyond the default circle: `sprite?: string` renders a raw image URL (for non-Pokemon sprites, e.g. SCOTUS justices); `feature?: string` takes a Pokemon vocab string ("Species" or "Species @ Item") and renders via the shared `SpriteImg` component (with item-icon overlay) inside a `<foreignObject>` — prefer `feature` for Pokemon nodes. Also takes `showLabels` (default true) and `spriteOpacity` (default 1).
- **Pokemon section figure.** A force-directed graph of the top ~32 species (one representative node each — the highest-marginal build), drawn from the live species+item `J` via `ModelContext`. Graph building, spring layout, and overlap relaxation live in `web/src/components/CouplingGraph.tsx`, shared with the Metagame Model page. The section owns only its filter UI (mode picker + `|J|` threshold slider) and passes a `filterEdge` callback to `CouplingGraph`. The prose's spin/parameter/team counts are read live from `model.V` / `model.nCorpusTeams`, not hardcoded.
