...
- **Internal research default model.** New research/notebook work should use the **weighted species@item fit** — `loaders.build_species_item_model(recency_tau=30.0, in_person_multiplier=2.0)` plus the current `SPECIES_ITEM_LR_LAMBDA`, the same fit family as the shipped `reg-m-a-species-item-weighted` artifact — unless there is a stated reason not to (species-only vocab when its statistical power is specifically needed, with weighted knobs `recency_tau=30.0, in_person_multiplier=3.0`; or unweighted fits for comparison baselines). Notebooks written before this convention may still fit unweighted production defaults; update them when touched.
