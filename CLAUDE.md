...
- **Internal research default model.** New research/notebook work should use the **weighted species@item fit** — `loaders.build_species_item_model(lam=3.0, recency_tau=30.0, in_person_multiplier=2.0)`, the same fit as the shipped `reg-m-a-species-item-weighted` artifact — unless there is a stated reason not to (species-only vocab when its statistical power is specifically needed, with weighted knobs `lam=30.0, recency_tau=30.0, in_person_multiplier=3.0`; or unweighted fits for comparison baselines). Notebooks written before this convention may still fit unweighted production defaults; update them when touched.
