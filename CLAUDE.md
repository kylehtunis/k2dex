...
- **Artifact schema v3 (factored `sites` + `tracks`)** — `meta.json` stores `sites: string[]` (distinct species, vocab order), `site_of: number[]` (per-feature site index), `tracks: {name, unique}[]` (attribute definitions; `[]` for species-only, `[{name:"item",unique:true}]` for species+item), and `track_values: (string|null)[][]` (per-feature per-track values). `species_of`/`item_of` are **removed** from the schema and reconstructed as convenience arrays by both loaders (`speciesOf[i]=sites[site_of[i]]`, `itemOf[i]=track_values[i][0] ?? null`). `site_features` (per-site feature grouping) is **not stored** — it is a pure projection of `site_of` both loaders derive at load. `vocab` stays (display string; drift is visible). `manifest.json` is schema_version 2 and carries per-model `tracks` for the UI toggle. `potts.py:load_fitted_model` and `web/src/sampler/model.ts:loadModel` both read v3 and rebuild the `(species, item)` view. `SUPPORTED_SCHEMA_VERSIONS = [3]` (no back-compat; `--recompute` rebuilds all artifacts).
