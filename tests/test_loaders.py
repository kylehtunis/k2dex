...
class TestWeightedVocabCutoff(unittest.TestCase):
    ...
    def test_single_heavy_team_cannot_mint_vocab(self) -> None:
        # Two tournaments each sized to clear the load-time team-count filter:
        # `n` old online teams (age 60d) + `n` recent in-person teams. At
        # tau=20 / mult=4 each recent team far outweighs an old one, so a pair
        # carried only by two old teams decays well below the cutoff of 2 even
        # though its raw count meets it. One recent team carries a unique (ghost)
        # pair (raw=1, below cutoff); two old teams carry a (stale) pair.
        n = MIN_TEAMS_PER_TOURNAMENT
        old_teams = [self._team(self._rotating(t, 6)) for t in range(n - 2)]
        old_teams += [self._team(self._rotating(t, 5) + [("StaleMon", "Stale Item", "Stale Ability")])
                      for t in range(2)]
        new_teams = [self._team(self._rotating(t, 6)) for t in range(n - 1)]
